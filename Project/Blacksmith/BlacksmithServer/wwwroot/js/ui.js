...
function renderTurn() {
    ...
    if (actionText) {
        actionText.textContent = turn
            ? `You used ${turn.playerSkill} ${turn.playerParam}. Opponent used ${turn.enemySkill} ${turn.enemyParam}.`
...
function renderWaitingState() {
    ...
    if (State.waitingForOpponent) {
        if (waitingEl) waitingEl.classList.remove('is-hidden');
        if (declareBtn) declareBtn.disabled = true;
        if (skillInput) skillInput.disabled = true;
    } else {
        if (waitingEl) waitingEl.classList.add('is-hidden');
        if (declareBtn) declareBtn.disabled = State.busy || !State.gameStarted || State.gameOver;
        if (skillInput) skillInput.disabled = false;
    }
}
...
function renderSnapshot(snapshot, options = {}) {
    ...
    if (modeBadge) modeBadge.textContent = 'PvP Online';
...
function updateBusyState() {
    ...
    if (declareBtn) declareBtn.disabled = State.busy || !State.gameStarted || isGameOver || State.waitingForOpponent;
    if (connectionState) {
        connectionState.textContent = State.connected ? 'Connected' : 'Disconnected';
        connectionState.className = State.connected ? 'status-pill' : 'status-pill muted';
    }
}
